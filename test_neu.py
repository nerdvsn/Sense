...
def test(testdata_path, depth_model = None, range_model = None,range_model2=None):
    ...
    Outputs = {
        ...
        # --- NEUE OUTPUTS FÜR STURZERKENNUNG ---
        'GT_timestamps': [],        # Zeitstempel für Geschwindigkeitsberechnung
        'Predicted_BBoxes': [],     # BBoxen (x, y, w, h)
        'Object_IDs': [],           # Tracking ID für konsistente Verfolgung
        # ----------------------------------------
    ...
    for sample_index in range(testset.len()):
        ...
        for i_box,ele in enumerate(matched_bbox):
            ...
            Outputs['Predicted_BBoxes'].append(pred_box)
...
if __name__ == "__main__":
    ...
    # Modelldateien (angenommen, 'os' und 'np' sind importiert)
    ...
    failed_file = []
    # Annahme: 'test_file_pathes' ist definiert und 'test' ist eine definierte Funktion
    for file_name in test_file_pathes: 
        try:
            ...
            # Konvertierung zu NumPy-Arrays (für Berechnungen)
            ...
            # --- KONTROLL-PRINTS FÜR NEUE PARAMETER ---
            print("\n--- Neue Parameter für Sturzerkennung (Kontrolle) ---")
            print("Anzahl der gespeicherten Instanzen:")
            # Drucke die Länge der Listen, um zu sehen, ob Daten vorhanden sind
            print(f"  GT_timestamps Länge: {len(GT_timestamps)}")
            print(f"  Predicted_BBoxes Länge: {len(Predicted_BBoxes)}")
            print(f"  Object_IDs Länge: {len(Object_IDs)}")
            print("\nErste 2 gespeicherte Elemente zur Strukturkontrolle:")
            # Drucke die ersten zwei Elemente zur Überprüfung des Datenformats
            print(f"  GT_timestamps [0:2]: {GT_timestamps[:2]}")
            print(f"  Predicted_BBoxes [0:2]: {Predicted_BBoxes[:2]}")
            print(f"  Object_IDs [0:2]: {Object_IDs[:2]}")
            print("------------------------------------------\n")
            sys.stdout.flush()
            # ---------------------------------------------
            ...
            # Annahme: 'DetectionMeasurements' ist eine definierte Funktion
            ...
            # --- Range Estimation Error (Range 2) ---
            ...
            # --- Depth Estimation Error (KF Smoothed) ---
            ...
            # --- Depth Estimation Error (Size-based & KF Smoothed) ---
            ...
            # Ein allgemeiner Fehlerfang, der den Dateinamen speichert und den Fehler ignoriert
            print(f"Error processing {file_name}: {e}")
            failed_file.append(file_name)
            pass # Fährt mit der nächsten Datei fort
